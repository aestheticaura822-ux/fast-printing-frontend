...
import sample1 from '../../assets/flyer.jpg';
import sample2 from '../../assets/flex1.jpg';
import sample3 from '../../assets/rollup.jpg';
...
const Flyers = () => {
  ...
  const [selectedPrint, setSelectedPrint] = useState('single');
  const [mainImage, setMainImage] = useState(flyersImg);
  ...
  const getPriceRange = () => {
    switch(quantity) {
      ...
      case 250: return 'PKR 4,000 - 6,000';
      case 500: return 'PKR 6,500 - 10,000';
      case 1000: return 'PKR 10,000 - 15,000';
...
{ icon: FileImage, title: 'Posters', price: 'From PKR 2,000', desc: 'Large format posters that make a big impact', image: postersImg, path: '/products/posters' },    { icon: CreditCard, title: 'Business Cards', price: 'From PKR 3,000', desc: 'Make a lasting first impression with premium business cards', image: businessCardImg, path: '/products/business-cards' }
  ];
  ...
  return (
    <div className="pt-20 overflow-x-hidden bg-gray-50">
        ...
        <div className="container mx-auto px-4">
          <div className="grid lg:grid-cols-2 gap-12">
            <div className="reveal">
              <div className="rounded-2xl overflow-hidden shadow-xl mb-4"><img src={mainImage} alt="Flyers" className="w-full h-auto object-cover" onError={(e) => { e.target.src = fallbackImage; }} /></div>
            ...
            <div className="reveal">
              ...
              <h1 className="text-4xl md:text-5xl font-bold text-gray-800 mb-4">Marketing <span className="text-red-600">Flyers</span></h1>
              <p className="text-gray-600 mb-6 leading-relaxed">Vibrant, high-impact flyers perfect for promotions, events, and announcements. Available in various sizes with fast turnaround times. Our flyers are printed on quality paper stock to ensure your message stands out.</p>
              <div className="grid grid-cols-2 gap-3 mb-6">
                {['Quick turnaround available', 'Single or double-sided', 'Various sizes A6 to A3', 'High-quality printing', 'Matte or gloss finish', 'Fast 1-2 day turnaround'].map((feature, idx) => (<div key={idx} className="flex items-center gap-2"><CheckCircle className="w-4 h-4 text-green-500" /><span className="text-sm text-gray-600">{feature}</span></div>))}
              </div>
              ...
              <div className="bg-white rounded-2xl p-6 shadow-md mb-6">
                <h3 className="text-lg font-bold text-gray-800 mb-4">Printing Options</h3>
                <div className="flex flex-wrap gap-3">{[{ id: 'single', label: 'Single-sided' }, { id: 'double', label: 'Double-sided' }].map((option) => (<button key={option.id} onClick={() => setSelectedPrint(option.id)} className={`px-4 py-2 rounded-lg text-sm font-semibold transition-all duration-300 ${selectedPrint === option.id ? 'bg-red-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>{option.label}</button>))}</div>
              </div>
      ...
      <section className="py-12 bg-white"><div className="container mx-auto px-4"><div className="text-center mb-10 reveal"><h2 className="text-3xl md:text-4xl font-bold text-gray-800 mb-3">Product <span className="text-red-600">Specifications</span></h2></div><div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-gray-50 rounded-2xl p-6 reveal"><Ruler className="w-6 h-6 text-red-600 mb-3" /><h3 className="text-xl font-bold mb-3">Sizes</h3><ul className="space-y-1 text-gray-600 text-sm"><li>A6 / A5 / A4 / A3</li><li>Custom sizes</li></ul></div>
        <div className="bg-gray-50 rounded-2xl p-6 reveal"><Layers className="w-6 h-6 text-red-600 mb-3" /><h3 className="text-xl font-bold mb-3">Paper Stock</h3><ul className="space-y-1 text-gray-600 text-sm"><li>80-170gsm Bond/Art Paper</li></ul></div>
        <div className="bg-gray-50 rounded-2xl p-6 reveal"><Printer className="w-6 h-6 text-red-600 mb-3" /><h3 className="text-xl font-bold mb-3">Printing</h3><ul className="space-y-1 text-gray-600 text-sm"><li>Single/Double-sided</li><li>Full color (CMYK)</li></ul></div>
        <div className="bg-gray-50 rounded-2xl p-6 reveal"><Gem className="w-6 h-6 text-red-600 mb-3" /><h3 className="text-xl font-bold mb-3">Finish</h3><ul className="space-y-1 text-gray-600 text-sm"><li>No lamination</li><li>Matte/Gloss lamination</li></ul></div>
      </div></div></section>

      <section className="py-12 bg-gray-50"><div className="container mx-auto px-4"><div className="text-center mb-10 reveal"><h2 className="text-3xl md:text-4xl font-bold text-gray-800 mb-3">Pricing <span className="text-red-600">Options</span></h2></div><div className="grid md:grid-cols-4 gap-6">{[{ qty: 100, price: 'PKR 2,000 - 3,500' }, { qty: 250, price: 'PKR 4,000 - 6,000' }, { qty: 500, price: 'PKR 6,500 - 10,000' }, { qty: 1000, price: 'PKR 10,000 - 15,000' }].map((option, idx) => (<div key={idx} className="bg-white rounded-2xl p-6 text-center shadow-lg hover:shadow-2xl transition-all duration-300 hover:-translate-y-2 reveal"><div className="text-3xl font-bold text-red-600 mb-2">{option.qty}</div><div className="text-sm text-gray-500 mb-2">Flyers</div><div className="text-xl font-bold text-gray-800">{option.price}</div></div>))}</div><p className="text-center text-sm text-gray-500 mt-6">* Prices may vary based on customization options</p></div></section>
      ...
      <section className="py-20 bg-gradient-to-r from-red-700 to-red-800"><div className="container mx-auto px-4 text-center"><h2 className="text-3xl md:text-4xl font-bold text-white mb-4">Ready to Get Started?</h2><p className="text-xl text-gray-200 mb-8">Request a custom quote for Flyers and we'll help you create something amazing.</p><div className="flex flex-wrap justify-center gap-4"><Link to="/contact"><button className="bg-yellow-500 hover:bg-yellow-600 text-gray-900 font-bold py-3 px-8 rounded-xl transition-all duration-300 hover:scale-105 shadow-xl flex items-center gap-2">Get Custom Quote <ArrowRight className="w-5 h-5" /></button></Link><Link to="/contact"><button className="border-2 border-white hover:bg-white hover:text-gray-900 text-white font-bold py-3 px-8 rounded-xl transition-all duration-300 hover:scale-105">Contact Us</button></Link></div></div></section>
    </div>
  );
};
