...
import { 
  ...
  Lock, CornerDownRight, Hash, Maximize, Wind, Sun, Grid
} from 'lucide-react';
...
import sample1 from '../../assets/window.jpg';
import sample2 from '../../assets/wall.jpg';
import sample3 from '../../assets/vehicle.jpg';
...
const WindowGraphics = () => {
  ...
  const [mainImage, setMainImage] = useState(windowGraphicsImg);
  ...
  const getPriceRange = () => {
    switch(quantity) {
      ...
      case 5: return 'PKR 12,000 - 25,000';
      case 10: return 'PKR 22,000 - 45,000';
      default: return 'Contact for quote';
    }
  };

  const relatedProducts = [
    { icon: Layout, title: 'Wall Graphics', price: 'From PKR 8,000', desc: 'Large-scale wall murals and graphics', image: wallGraphicsImg, path: '/products/wall-graphics' },
  ...
  return (
    <div className="pt-20 overflow-x-hidden bg-gray-50">
        ...
        <div className="container mx-auto px-4">
          <div className="grid lg:grid-cols-2 gap-12">
            <div className="reveal">
              <div className="rounded-2xl overflow-hidden shadow-xl mb-4"><img src={mainImage} alt="Window Graphics" className="w-full h-auto object-cover" onError={(e) => { e.target.src = fallbackImage; }} /></div>
            ...
            <div className="reveal">
              ...
              <h1 className="text-4xl md:text-5xl font-bold text-gray-800 mb-4">Window <span className="text-red-600">Graphics</span></h1>
              <p className="text-gray-600 mb-6 leading-relaxed">Eye-catching window graphics and decals for storefronts, offices, and vehicles. Available in various materials including perforated vinyl for one-way visibility. Perfect for promotions, branding, and privacy.</p>
              <div className="grid grid-cols-2 gap-3 mb-6">
                {['Perforated vinyl (one-way)', 'Full-color printing', 'UV-resistant inks', 'Easy application', 'Removable adhesive', 'Fast 3-5 day turnaround'].map((feature, idx) => (<div key={idx} className="flex items-center gap-2"><CheckCircle className="w-4 h-4 text-green-500" /><span className="text-sm text-gray-600">{feature}</span></div>))}
              </div>
              <div className="bg-white rounded-2xl p-6 shadow-md mb-6">
                <h3 className="text-lg font-bold text-gray-800 mb-4">Select Quantity</h3>
                <div className="flex flex-wrap gap-3">{[1, 5, 10].map((qty) => (<button key={qty} onClick={() => setQuantity(qty)} className={`px-6 py-2 rounded-lg font-semibold transition-all duration-300 ${quantity === qty ? 'bg-red-600 text-white shadow-lg' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>{qty}</button>))}</div>
      ...
      <section className="py-12 bg-white"><div className="container mx-auto px-4"><div className="text-center mb-10 reveal"><h2 className="text-3xl md:text-4xl font-bold text-gray-800 mb-3">Product <span className="text-red-600">Specifications</span></h2></div><div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-gray-50 rounded-2xl p-6 reveal"><Layers className="w-6 h-6 text-red-600 mb-3" /><h3 className="text-xl font-bold mb-3">Materials</h3><ul className="space-y-1 text-gray-600 text-sm"><li>Perforated vinyl (50/50)</li><li>Clear / White / Frosted</li><li>Etched glass film</li></ul></div>
        <div className="bg-gray-50 rounded-2xl p-6 reveal"><Shield className="w-6 h-6 text-red-600 mb-3" /><h3 className="text-xl font-bold mb-3">Adhesive Type</h3><ul className="space-y-1 text-gray-600 text-sm"><li>Permanent / Removable</li><li>Static cling (no adhesive)</li><li>Low-tack adhesive</li></ul></div>
        <div className="bg-gray-50 rounded-2xl p-6 reveal"><Layout className="w-6 h-6 text-red-600 mb-3" /><h3 className="text-xl font-bold mb-3">Application</h3><ul className="space-y-1 text-gray-600 text-sm"><li>Interior / Exterior window</li><li>Storefront / Vehicle window</li><li>Glass doors</li></ul></div>
        <div className="bg-gray-50 rounded-2xl p-6 reveal"><Ruler className="w-6 h-6 text-red-600 mb-3" /><h3 className="text-xl font-bold mb-3">Sizes</h3><ul className="space-y-1 text-gray-600 text-sm"><li>Small / Medium / Large</li><li>Full window coverage</li><li>Custom sizes available</li></ul></div>
      </div></div></section>

      <section className="py-12 bg-gray-50"><div className="container mx-auto px-4"><div className="text-center mb-10 reveal"><h2 className="text-3xl md:text-4xl font-bold text-gray-800 mb-3">Pricing <span className="text-red-600">Options</span></h2></div><div className="grid md:grid-cols-4 gap-6">{[{ qty: 1, price: 'PKR 3,000 - 6,000' }, { qty: 5, price: 'PKR 12,000 - 25,000' }, { qty: 10, price: 'PKR 22,000 - 45,000' }, { qty: 'Custom', price: 'Contact for quote' }].map((option, idx) => (<div key={idx} className="bg-white rounded-2xl p-6 text-center shadow-lg hover:shadow-2xl transition-all duration-300 hover:-translate-y-2 reveal"><div className="text-3xl font-bold text-red-600 mb-2">{option.qty}</div><div className="text-sm text-gray-500 mb-2">Graphics</div><div className="text-xl font-bold text-gray-800">{option.price}</div></div>))}</div><p className="text-center text-sm text-gray-500 mt-6">* Prices may vary based on customization options</p></div></section>
      ...
      <section className="py-20 bg-gradient-to-r from-red-700 to-red-800"><div className="container mx-auto px-4 text-center"><h2 className="text-3xl md:text-4xl font-bold text-white mb-4">Ready to Get Started?</h2><p className="text-xl text-gray-200 mb-8">Request a custom quote for Window Graphics and we'll help you create something amazing.</p><div className="flex flex-wrap justify-center gap-4"><Link to="/contact"><button className="bg-yellow-500 hover:bg-yellow-600 text-gray-900 font-bold py-3 px-8 rounded-xl transition-all duration-300 hover:scale-105 shadow-xl flex items-center gap-2">Get Custom Quote <ArrowRight className="w-5 h-5" /></button></Link><Link to="/contact"><button className="border-2 border-white hover:bg-white hover:text-gray-900 text-white font-bold py-3 px-8 rounded-xl transition-all duration-300 hover:scale-105">Contact Us</button></Link></div></div></section>
    </div>
  );
};
