...
import { 
  ...
  Lock, CornerDownRight, Hash, Maximize, Crown
} from 'lucide-react';
...
import sample1 from '../../assets/bluecard.jpg';
import sample2 from '../../assets/ameen-card.jpg';
import sample3 from '../../assets/dholki.jpg';
import sample4 from '../../assets/scroll-wedding-card.jpg';

const fallbackImage = 'https://images.unsplash.com/photo-1519741497674-611481863552?w=600';

const WeddingCards = () => {
  const [quantity, setQuantity] = useState(100);
  const [mainImage, setMainImage] = useState(weddingCardsImg);
  ...
  const getPriceRange = () => {
    switch(quantity) {
      case 50: return 'PKR 6,000 - 12,000';
      case 100: return 'PKR 10,000 - 20,000';
      case 150: return 'PKR 14,000 - 28,000';
      case 200: return 'PKR 18,000 - 35,000';
      default: return 'PKR 6,000 - 12,000';
    }
  };

  const invitationCategories = [
    { title: 'STYLE', items: ['Classic', 'Modern', 'Luxury'], price: 'From PKR 150' },
    { title: 'WEDDING STATIONERY', items: ['Invitations', 'Save the Date', 'Thank You'], price: 'From PKR 120' },
    { title: 'ON YOUR WEDDING DAY', items: ['Programs', 'Menus', 'Place Cards'], price: 'From PKR 200' }
  ];

  const InvitationCategoryCard = ({ category }) => (
    <div className="bg-white rounded-2xl p-6 text-center shadow-lg hover:shadow-2xl transition-all duration-300 hover:-translate-y-2">
      ...
      <p className="text-red-600 font-semibold mb-4">{category.price}</p>
      <div className="flex flex-wrap justify-center gap-2">
        {category.items.map((item, idx) => (<span key={idx} className="text-xs bg-gray-100 text-gray-600 px-2 py-1 rounded-full">{item}</span>))}
      </div>
    </div>
  );
  ...
  const relatedProducts = [
    ...
    { icon: Award, title: 'Certificates', price: 'From PKR 5,000', desc: 'Professional certificates and awards', image: certificatesImg, path: '/products/certificates' }
  ];
  ...
  return (
    <div className="pt-20 overflow-x-hidden bg-gray-50">
        ...
        <div className="container mx-auto px-4">
          <div className="grid lg:grid-cols-2 gap-12">
            <div className="reveal">
              <div className="rounded-2xl overflow-hidden shadow-xl mb-4"><img src={mainImage} alt="Wedding Cards" className="w-full h-auto object-cover" onError={(e) => { e.target.src = fallbackImage; }} /></div>
            ...
            <div className="reveal">
              ...
              <h1 className="text-4xl md:text-5xl font-bold text-gray-800 mb-4">Wedding <span className="text-red-600">Cards</span></h1>
              <p className="text-gray-600 mb-6 leading-relaxed">Beautiful custom wedding invitations and cards for your special day. Available in various designs, sizes, and luxury finishes. From save-the-date cards to thank you notes, we create elegant stationery that sets the perfect tone.</p>
              <div className="grid grid-cols-2 gap-3 mb-6">
                {['Custom design services', 'Luxury paper stocks', 'Foil stamping available', 'Embossing & debossing', 'Envelope printing included', 'Fast 7-10 day turnaround'].map((feature, idx) => (<div key={idx} className="flex items-center gap-2"><CheckCircle className="w-4 h-4 text-green-500" /><span className="text-sm text-gray-600">{feature}</span></div>))}
              </div>
              <div className="bg-white rounded-2xl p-6 shadow-md mb-6">
                <h3 className="text-lg font-bold text-gray-800 mb-4">Select Quantity</h3>
                <div className="flex flex-wrap gap-3">{[50, 100, 150, 200].map((qty) => (<button key={qty} onClick={() => setQuantity(qty)} className={`px-6 py-2 rounded-lg font-semibold transition-all duration-300 ${quantity === qty ? 'bg-red-600 text-white shadow-lg' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>{qty}</button>))}</div>
      ...
      <section className="py-12 bg-white"><div className="container mx-auto px-4"><div className="text-center mb-10 reveal"><h2 className="text-3xl md:text-4xl font-bold text-gray-800 mb-3">Product <span className="text-red-600">Specifications</span></h2></div><div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-gray-50 rounded-2xl p-6 reveal"><Crown className="w-6 h-6 text-red-600 mb-3" /><h3 className="text-xl font-bold mb-3">Card Types</h3><ul className="space-y-1 text-gray-600 text-sm"><li>Wedding invitations / Save-the-date</li><li>RSVP / Thank you cards</li><li>Menu / Program cards</li></ul></div>
        <div className="bg-gray-50 rounded-2xl p-6 reveal"><Ruler className="w-6 h-6 text-red-600 mb-3" /><h3 className="text-xl font-bold mb-3">Sizes</h3><ul className="space-y-1 text-gray-600 text-sm"><li>5" x 7" / 4" x 6"</li><li>5.5" x 8.5" / Square (5"x5")</li><li>Custom sizes available</li></ul></div>
        <div className="bg-gray-50 rounded-2xl p-6 reveal"><Layers className="w-6 h-6 text-red-600 mb-3" /><h3 className="text-xl font-bold mb-3">Paper Stock</h3><ul className="space-y-1 text-gray-600 text-sm"><li>Premium cardstock (18pt-24pt)</li><li>Pearl shimmer / Linen textured</li><li>Cotton / Metallic / Handmade</li></ul></div>
        <div className="bg-gray-50 rounded-2xl p-6 reveal"><Gem className="w-6 h-6 text-red-600 mb-3" /><h3 className="text-xl font-bold mb-3">Finishing</h3><ul className="space-y-1 text-gray-600 text-sm"><li>Foil stamping (Gold/Silver/Rose)</li><li>Embossing/Debossing / Laser cutting</li><li>Ribbon attachments / Wax seals</li></ul></div>
      </div></div></section>

      <section className="py-12 bg-gray-50"><div className="container mx-auto px-4"><div className="text-center mb-10 reveal"><h2 className="text-3xl md:text-4xl font-bold text-gray-800 mb-3">Pricing <span className="text-red-600">Options</span></h2></div><div className="grid md:grid-cols-4 gap-6">{[{ qty: 50, price: 'PKR 6,000 - 12,000' }, { qty: 100, price: 'PKR 10,000 - 20,000' }, { qty: 150, price: 'PKR 14,000 - 28,000' }, { qty: 200, price: 'PKR 18,000 - 35,000' }].map((option, idx) => (<div key={idx} className="bg-white rounded-2xl p-6 text-center shadow-lg hover:shadow-2xl transition-all duration-300 hover:-translate-y-2 reveal"><div className="text-3xl font-bold text-red-600 mb-2">{option.qty}</div><div className="text-sm text-gray-500 mb-2">Cards</div><div className="text-xl font-bold text-gray-800">{option.price}</div></div>))}</div><p className="text-center text-sm text-gray-500 mt-6">* Prices may vary based on customization options</p></div></section>

      <section className="py-12 bg-white"><div className="container mx-auto px-4"><div className="text-center mb-10 reveal"><h2 className="text-3xl md:text-4xl font-bold text-gray-800 mb-3">Invitation <span className="text-red-600">Categories</span></h2></div><div className="grid md:grid-cols-3 gap-6">{invitationCategories.map((category, idx) => (<InvitationCategoryCard key={idx} category={category} />))}</div></div></section>
      ...
      <section className="py-20 bg-gradient-to-r from-red-700 to-red-800"><div className="container mx-auto px-4 text-center"><h2 className="text-3xl md:text-4xl font-bold text-white mb-4">Ready to Get Started?</h2><p className="text-xl text-gray-200 mb-8">Request a custom quote for Wedding Cards and we'll help you create something amazing.</p><div className="flex flex-wrap justify-center gap-4"><Link to="/contact"><button className="bg-yellow-500 hover:bg-yellow-600 text-gray-900 font-bold py-3 px-8 rounded-xl transition-all duration-300 hover:scale-105 shadow-xl flex items-center gap-2">Get Custom Quote <ArrowRight className="w-5 h-5" /></button></Link><Link to="/contact"><button className="border-2 border-white hover:bg-white hover:text-gray-900 text-white font-bold py-3 px-8 rounded-xl transition-all duration-300 hover:scale-105">Contact Us</button></Link></div></div></section>
    </div>
  );
};
