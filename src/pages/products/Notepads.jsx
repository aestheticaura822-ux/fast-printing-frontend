...
import { 
  ...
  Lock, CornerDownRight, Hash, Maximize, Notebook, Calendar
} from 'lucide-react';
...
import sample1 from '../../assets/notebook.jpg';
import sample2 from '../../assets/book.jpg';
import sample3 from '../../assets/marksales.jpg';
import sample4 from '../../assets/markcovers.jpg';

const fallbackImage = 'https://images.unsplash.com/photo-1586339949216-35c2747cc36d?w=600';

const Notepads = () => {
  const [quantity, setQuantity] = useState(50);
  const [mainImage, setMainImage] = useState(notepadsImg);
  ...
  const getPriceRange = () => {
    switch(quantity) {
      case 25: return 'PKR 4,000 - 7,000';
      case 50: return 'PKR 7,000 - 12,000';
      case 100: return 'PKR 12,000 - 22,000';
      case 250: return 'PKR 28,000 - 50,000';
      default: return 'PKR 4,000 - 7,000';
    }
  };
  ...
  const relatedProducts = [
    ...
    { icon: Calendar, title: 'Calendars', price: 'From PKR 8,000', desc: 'Custom calendars for promotional use', image: calendarsImg, path: '/products/calendars' }
  ];
  ...
  return (
    <div className="pt-20 overflow-x-hidden bg-gray-50">
        ...
        <div className="container mx-auto px-4">
          <div className="grid lg:grid-cols-2 gap-12">
            <div className="reveal">
              <div className="rounded-2xl overflow-hidden shadow-xl mb-4"><img src={mainImage} alt="Notepads" className="w-full h-auto object-cover" onError={(e) => { e.target.src = fallbackImage; }} /></div>
            ...
            <div className="reveal">
              <div className="inline-flex items-center gap-2 bg-red-100 text-red-600 rounded-full px-3 py-1 text-sm font-semibold mb-4"><Star className="w-4 h-4 fill-red-600" /> Corporate Gift</div>
              <h1 className="text-4xl md:text-5xl font-bold text-gray-800 mb-4">Branded <span className="text-red-600">Notepads</span></h1>
              <p className="text-gray-600 mb-6 leading-relaxed">Custom branded notepads perfect for corporate gifts, office supplies, and promotional items. Available in various sizes with multiple binding options. Keep your brand in front of your clients daily.</p>
              <div className="grid grid-cols-2 gap-3 mb-6">
                {['Custom header/footer printing', 'Various page counts', 'Cardboard backing', 'Multiple binding options', 'Perforated pages available', 'Fast 5-7 day turnaround'].map((feature, idx) => (<div key={idx} className="flex items-center gap-2"><CheckCircle className="w-4 h-4 text-green-500" /><span className="text-sm text-gray-600">{feature}</span></div>))}
              </div>
              <div className="bg-white rounded-2xl p-6 shadow-md mb-6">
                <h3 className="text-lg font-bold text-gray-800 mb-4">Select Quantity (Pads)</h3>
      ...
      <section className="py-12 bg-white"><div className="container mx-auto px-4"><div className="text-center mb-10 reveal"><h2 className="text-3xl md:text-4xl font-bold text-gray-800 mb-3">Product <span className="text-red-600">Specifications</span></h2></div><div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-gray-50 rounded-2xl p-6 reveal"><Ruler className="w-6 h-6 text-red-600 mb-3" /><h3 className="text-xl font-bold mb-3">Sizes</h3><ul className="space-y-1 text-gray-600 text-sm"><li>A4 / A5 / Letter</li><li>Legal (14" x 8.5")</li><li>Custom sizes available</li></ul></div>
        <div className="bg-gray-50 rounded-2xl p-6 reveal"><Lock className="w-6 h-6 text-red-600 mb-3" /><h3 className="text-xl font-bold mb-3">Binding</h3><ul className="space-y-1 text-gray-600 text-sm"><li>Top glue pad / Side glue pad</li><li>Spiral/Wire-O / Perfect bound</li></ul></div>
        <div className="bg-gray-50 rounded-2xl p-6 reveal"><Layers className="w-6 h-6 text-red-600 mb-3" /><h3 className="text-xl font-bold mb-3">Paper Stock</h3><ul className="space-y-1 text-gray-600 text-sm"><li>60-80gsm Bond Paper</li><li>Recycled paper</li></ul></div>
        <div className="bg-gray-50 rounded-2xl p-6 reveal"><Notebook className="w-6 h-6 text-red-600 mb-3" /><h3 className="text-xl font-bold mb-3">Options</h3><ul className="space-y-1 text-gray-600 text-sm"><li>Blank / Lined / Grid pages</li><li>Perforated pages</li><li>Cardboard backing / Numbering</li></ul></div>
      </div></div></section>

      <section className="py-12 bg-gray-50"><div className="container mx-auto px-4"><div className="text-center mb-10 reveal"><h2 className="text-3xl md:text-4xl font-bold text-gray-800 mb-3">Pricing <span className="text-red-600">Options</span></h2></div><div className="grid md:grid-cols-4 gap-6">{[{ qty: 25, price: 'PKR 4,000 - 7,000' }, { qty: 50, price: 'PKR 7,000 - 12,000' }, { qty: 100, price: 'PKR 12,000 - 22,000' }, { qty: 250, price: 'PKR 28,000 - 50,000' }].map((option, idx) => (<div key={idx} className="bg-white rounded-2xl p-6 text-center shadow-lg hover:shadow-2xl transition-all duration-300 hover:-translate-y-2 reveal"><div className="text-3xl font-bold text-red-600 mb-2">{option.qty}</div><div className="text-sm text-gray-500 mb-2">Pads</div><div className="text-xl font-bold text-gray-800">{option.price}</div></div>))}</div><p className="text-center text-sm text-gray-500 mt-6">* Prices may vary based on customization options</p></div></section>
      ...
      <section className="py-20 bg-gradient-to-r from-red-700 to-red-800"><div className="container mx-auto px-4 text-center"><h2 className="text-3xl md:text-4xl font-bold text-white mb-4">Ready to Get Started?</h2><p className="text-xl text-gray-200 mb-8">Request a custom quote for Notepads and we'll help you create something amazing.</p><div className="flex flex-wrap justify-center gap-4"><Link to="/contact"><button className="bg-yellow-500 hover:bg-yellow-600 text-gray-900 font-bold py-3 px-8 rounded-xl transition-all duration-300 hover:scale-105 shadow-xl flex items-center gap-2">Get Custom Quote <ArrowRight className="w-5 h-5" /></button></Link><Link to="/contact"><button className="border-2 border-white hover:bg-white hover:text-gray-900 text-white font-bold py-3 px-8 rounded-xl transition-all duration-300 hover:scale-105">Contact Us</button></Link></div></div></section>
    </div>
  );
};
